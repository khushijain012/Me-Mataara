...
export function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // The device-local account only exists under the mock provider; against the
    // backend, any registered user can sign in from any device.
    if (auth.kind === 'mock' && !profile) {
    ...
    try {
      ...
      if (ok) {
        ...
        // A clean 401 — credentials didn't match an account.
        setError('Incorrect mobile number or password. Please try again.')
      }
    } catch (err) {
      // Network / server problems carry their own message from the provider.
