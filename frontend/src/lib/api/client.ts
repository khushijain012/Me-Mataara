...
/**
 * Resolve an image/file reference for use in `<img src>` or a download link.
 *
 * The backend hands out API-relative paths (`/concerns/photos/<id>?t=…`) because
 * the bytes live in its file storage, not in the payload. Offline records still
 * carry the on-device data URL they were captured as, and absolute URLs may come
 * from elsewhere — both pass straight through.
 */
export function resolveFileUrl(src: string): string {
