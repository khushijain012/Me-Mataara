...
import { resolveStorageKey, writeStorageFile } from '../lib/fileStorage.js'
import { authenticate, currentMember, requireRole } from '../middleware/auth.js'
import { badRequest, notFound } from '../lib/httpError.js'

// Admin-uploaded content library (#4, #5). Files land in <storage root>/<type>/.
...
mediaRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const storageKey = path.posix.join(mediaType.storageFolder, filename)
...
mediaRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const full = resolveStorageKey(asset.storageKey)
