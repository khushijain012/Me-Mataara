...
import { concernPhotosRouter } from './routes/concernPhotos.js'
...
// Photos are uploaded as data URLs inside the concern payload (the shape that
// survives in the PWA's offline queue), so allow a generous JSON body. The bytes
// are written to disk on arrival — see src/lib/photos.ts.
...
api.use('/concerns/photos', concernPhotosRouter) // before /concerns: not behind bearer auth
