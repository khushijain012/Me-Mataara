...
export function toConcern(
  ...
  // Photos are signed URLs into GET /concerns/photos/:id — the bytes stay on
  // disk and never travel inside this payload.
  const riskIds = [...c.risks]
    .sort((a, b) => Number(b.isPrimary) - Number(a.isPrimary))
    .map((r) => r.categoryId)
  const photos = [...c.photos].sort((a, b) => a.sortOrder - b.sortOrder).map((p) => photoUrl(p.id))
  return {
    id: c.id,
    ref: c.ref,
